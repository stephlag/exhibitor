...
public class StandardProcessOperations implements ProcessOperations
{
    ...
    public void cleanupInstance() throws Exception
    {
        ...
        ProcessBuilder builder = new ProcessBuilder
                (
                        "java",
                        "-cp",
                        String.format("%s:%s:%s", details.zooKeeperJarPath, details.logPaths,
                                details.configDirectory.getPath()),
        ...
        exhibitor.getProcessMonitor().monitor(ProcessTypes.CLEANUP, builder.start(), "Cleanup task completed",
                ProcessMonitor.Mode.DESTROY_ON_INTERRUPT, ProcessMonitor.Streams.BOTH);
    }
    ...
    private void prepConfigFile(Details details) throws IOException
    {
        ...
        String portSpec = String.format(":%d:%d", usState.getConfig().getInt(IntConfigs.CONNECT_PORT), usState
                .getConfig().getInt(IntConfigs.ELECTION_PORT));
        for (ServerSpec spec : usState.getServerList().getSpecs())
        {
            localProperties.setProperty("server." + spec.getServerId(), spec.getHostname() + portSpec
                    + spec.getServerType().getZookeeperConfigValue());
        }
